...
def check_auth():
    try:
        VALID_TOKENS = {
            "client1": os.getenv("TOKEN_1", "token-abc123"),
            "client2": os.getenv("TOKEN_2", "token-xyz789")
        }
...
def reset_processing():
    ...
    st.session_state.email_check_attempted = False
    st.rerun()
...
def get_sender_info():
    """Manual sender info form (only shown after email check)"""
    with st.form("sender_info_form"):
        st.subheader("Sender Information (Manual Entry)")
        sender_name = st.text_input("Sender Name*", max_chars=50)
        sender_email = st.text_input("Sender Email", max_chars=100)
        
        submitted = st.form_submit_button("Submit Sender Info")
        if submitted:
            if not sender_name:
...
def verify_details(extracted_data):
    """Form to verify/edit extracted details"""
...
if st.session_state.processing_stage == "upload":
    ...
    if uploaded_file:
        with st.spinner("Extracting receipt data..."):
            ...
            if st.session_state.file_hash not in st.session_state.processed_files:
                try:
                    text = "\n".join(
                ...
                st.warning("This receipt has already been processed")
                reset_processing()
    ...
    if not st.session_state.email_check_attempted:
        if st.button("Check Emails for Sender Info"):
            with st.spinner("Checking emails..."):
                try:
                    ...
                    st.session_state.email_check_attempted = True
                    st.rerun()
                except Exception as e:
    ...
    st.subheader("4. Submit to Records")
    
    if st.session_state.sender_info and st.session_state.receipt_details:
        complete_data = {
            **st.session_state.sender_info,
            **st.session_state.receipt_details
        }
        
        filename = f"{sanitize_filename(complete_data['name'])}_{sanitize_filename(complete_data['item'])}_{sanitize_filename(complete_data['receipt_number'])}_{complete_data['cost']}.pdf"
        ...
        if st.button("Confirm and Submit"):
            with st.spinner("Saving..."):
                try:
                    ...
                    with open(os.path.join("receipts", filename), "wb") as f:
                        f.write(st.session_state.current_receipt)
                    ...
                    if processor.append_to_sheet([
                        ...
                        st.session_state.processed_files.add(st.session_state.file_hash)
    ...
    filename = f"{sanitize_filename(st.session_state.sender_info['name'])}_{sanitize_filename(st.session_state.receipt_details['item'])}_{sanitize_filename(st.session_state.receipt_details['receipt_number'])}_{st.session_state.receipt_details['cost']}.pdf"
